...
export default function EditProviderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [hasSecrets, setHasSecrets] = useState({ client_secret: false, refresh_token: false });
    ...
    const fetchProvider = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setHasSecrets({
            client_secret: data.has_client_secret ?? false,
            refresh_token: data.has_refresh_token ?? false,
          });
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErrors({ general: ["Failed to update provider."] });
      }
    } catch {
    ...
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted mt-1">Update provider settings</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-card rounded-xl border border-border p-6 space-y-5">
        ...
        <FormField label="Client Secret" error={errors.client_secret}>
          <input
            ...
            className="form-input"
          />
          {hasSecrets.client_secret && (
            <p className="text-xs text-muted mt-1">Clear and type new value to change</p>
          )}
        </FormField>
